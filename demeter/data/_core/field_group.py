...
def getParcelGroupAncestors(
    ...
    with recursive leaf as (
      ...
      where G.parcel_group_id = %(parcel_group)s
      ...
      group by A.leaf_id
  """
    cursor.execute(stmt, parcel_group())
...
def getOrgParcels(
    ...
    with recursive ancestry as (
     ...
     from leaf L
     left join test_mlops.parcel F on F.parcel_group_id = L.parcel_group_id
...
def getParcelGroupParcels(
  ...
  with recursive ancestor as (
    ...
    from descendant D
    left join test_mlops.parcel F on D.parcel_group_id = F.parcel_group_id
