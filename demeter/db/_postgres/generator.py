...
class SQLGenerator:
  ...
  def getInsertReturnIdFunction(self, table : Type[I]) -> ReturnId[I]:
    ...
    return self._fix_annotations(partial(insertAndReturnId, table_name), table.__name__)
  ...
  def getInsertReturnKeyFunction(self, table : Type[S], key : Type[SK]) -> ReturnKey[S, SK]:
    table_name = self.key_table_lookup[table]
    fn = cast(ReturnKey[S, SK], insertAndReturnKey(table_name, key))
    return self._fix_annotations(fn, table.__name__, key.__name__)

  def getMaybeIdFunction(self, table : Type[T]) -> Callable[[Any, AnyIdTable], Optional[TableId]]:
    table_name = self.id_table_lookup[table]
  ...
  def getTableByKeyFunction(self,
    ...
    return self._fix_annotations(partial(gtbk, table_name), key_table.__name__, table.__name__)
