with latest_function as (
  ...
  select F.function_id,
         jsonb_agg(
           jsonb_build_object('type_name', OT.type_name,
                              'type_category', OT.type_category
                             )
         ) as observation_types
  from latest_function F
  join observation_parameter OP on F.function_id = OP.function_id
  join observation_type OT on OP.observation_type_id = OT.observation_type_id
  ...
  group by F.function_id
         ...
         coalesce(OI.observation_types, '[]'::jsonb) as observation_inputs,
  ...
  from latest_function F
  left join observation_inputs OI on F.function_id = OI.function_id
