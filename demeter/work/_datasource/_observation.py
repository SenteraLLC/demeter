...
def queryObservationByKey(
    ...
    stmt = """select V.*, U.unit, U.observation_type_id
            from observation_value V, unit_type U
    ...
    observation_value: List[data.ObservationValue] = sqlToTypedDict(results, data.ObservationValue)
    unit_type: List[data.UnitType] = sqlToTypedDict(results, data.UnitType)

    return zip(observation_value, unit_type)
