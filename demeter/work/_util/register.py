...
def getSignature(
    ...
    def unpackS3Type(
        s3_type_and_subtype: Tuple[task.S3Type, Optional[task.TaggedS3SubType]]
    ) -> task.S3TypeSignature:
        ...
        if maybe_tagged_s3_subtype is not None:
            ...
            if tag == task.S3TypeDataFrame:
                ...
                raise Exception(
                    f"Unhandled write for S3 sub-type: {tag} -> {s3_subtype}"
                )
        return s3_type, None
    ...
    s3_inputs = [
        unpackS3Type(task.getS3Type(cursor, i)) for i in input_types["s3_type_ids"]
    ]
    s3_outputs = [
        unpackS3Type(task.getS3Type(cursor, s3_type_id))
        for (_, s3_type_id) in output_types.values()
    ]
    ...
    return task.FunctionSignature(
        ...
        observation_inputs=[
            data.getObservationType(cursor, i)
...
def diffSignatures(
    ...
    if maybe_latest_signature is not None:
        ...
        if len(diff) > 0:
            ...
            major = function.major
            raise Exception(
                f"Cannot register function '{name}' with major {major}:\n Diff: {diff}"
            )
    return True
...
def registerFunction(
    ...
    function_id, new_minor = insertFunctionTypes(
        cursor, function, input_types, keyword_types, output_types
    )

    candidate_signature = getSignature(
        cursor, function, input_types, keyword_types, output_types
    )
