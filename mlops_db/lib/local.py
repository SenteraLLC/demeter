...
def _load(cursor      : Any,
  ...
  local_type_ids : Set[int] = set()
  for local_type in local_types:
    local_type_id = getMaybeLocalTypeId(cursor, local_type)
    if local_type_id is None:
      raise Exception(f"Failed to find ID for local type: {local_type}")
    local_type_ids.add(local_type_id)
  stmt = """select *
            from local_value V, unit_type U
            where V.unit_type_id = U.unit_type_id and U.local_type_id = any(%s) and
  ...
  args = (local_type_ids, geom_id, field_id, start_date, end_date)
...
def load(cursor      : Any,
    ...
    for k in keys:
      partial_results = _load(cursor, k, local_types)
