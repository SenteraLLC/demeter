...
def grammar():
    ...
    quoted_default_value = (
        CaselessLiteral("DEFAULT") + quoted_string + OneOrMore(CharsNotIn(", \n\t"))
    )
    quoted_default_value.setParseAction(quoted_default_value_act)

    field_def = OneOrMore(
        quoted_default_value | Word(alphanums + "_\"'`:-/[].") | parenthesis
    )
    ...
    fkey_cols = Word(alphanums + "._") + ZeroOrMore(
        Suppress(",") + Word(alphanums + "._")
    )

    add_fkey_def = (
        CaselessLiteral("ALTER")
        + CaselessLiteral("TABLE")
        ...
        + ")"
        + "REFERENCES"
        + tablename_def.setResultsName("fkTable")
        + "("
    ...
    return OneOrMore(
        comment_def | create_table_def | add_fkey_def | other_statement_def
    )
