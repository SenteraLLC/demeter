...
def run_daily_weather(
    ...
    logging.info("Entering UPDATE step")
    ...
    if n_requests_available > 0:
        ...
        logging.info("  UPDATING %s CELL IDS", len(gdf_update))
    ...
    logging.info("Entering ADD step")
    n_requests_available = get_n_requests_remaining_for_demeter(conn)
    if n_requests_available > 0:
        gdf_add = get_gdf_for_add(conn)
        logging.info("  ADDING %s CELL IDS", len(gdf_add))
    ...
    if fill:
        logging.info("Entering FILL step")
        n_requests_available = get_n_requests_remaining_for_demeter(conn)
        if n_requests_available > 0:
            gdf_fill = get_gdf_for_fill(conn)
            logging.info("  FILLING %s ROWS", len(gdf_fill))
